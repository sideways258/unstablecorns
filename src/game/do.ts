...
function leave(G: UnstableUnicornsGame, ctx: Ctx, param: ParamLeave) {
    ...
    const on = (isUnicorn(G.deck[param.cardID])
        ? [...G.stable[param.playerID], ...G.upgradeDowngradeStable[param.playerID]]
        : []
    ).map(c => G.deck[c]).filter(s => s.on && s.on.filter(o => o.trigger === "unicorn_leaves_your_stable").length > 0);
    on.forEach(card => {
...
export function enter(G: UnstableUnicornsGame, ctx: Ctx, param: ParamEnter) {
    ...
    const on = (isUnicorn(card)
        ? [...G.stable[param.playerID], ...G.upgradeDowngradeStable[param.playerID]]
        : []
    ).map(c => G.deck[c]).filter(s => s.on && s.on.filter(o => o.trigger === "unicorn_enters_your_stable").length > 0);
    on.forEach(card => {
        // all unicorns are basic
        // trigger no effect
        if (G.playerEffects[param.playerID].find(s => s.effect.key === "my_unicorns_are_basic")) {
...
export function findSacrificeTargets(G: UnstableUnicornsGame, ctx: Ctx, protagonist: PlayerID, info: DoSacrificeInfo): SacrificeTarget[] {
    ...
    if (info.type === "any") {
        ...
        targets = [
            ...G.stable[protagonist],
            ...G.upgradeDowngradeStable[protagonist],
        ].map(c => ({ cardID: c }));
    }
