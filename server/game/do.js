...
function canEnter(G, ctx, param) {
    ...
    if (card.type !== "upgrade" && card.type !== "downgrade"
        && game_1._countUnicorns(G, param.playerID) >= constants_1.CONSTANTS.stableSeats) {
        return false;
    }
        ...
        if (info.type === "unicorn") {
            ...
            G.stable[pl.id].forEach(function (cid) {
                ...
                if (card_1.isUnicorn(card)) {
                    ...
                    if (pl.id !== protagonist && G.playerEffects[pl.id].find(function (s) { return s.effect.key === "your_unicorns_cannot_be_destroyed"; })) {
                        return;
                    }
                    targets.push({ playerID: pl.id, cardID: cid });
                }
